...
function MetadataModal({ isOpen, closeModal, fetchPatchInfo }) {
    ...
    return (
        <div id="metadata-modal" className={`fixed top-0 left-0 w-full h-full bg-gray-900 bg-opacity-75 flex justify-center items-center transition-opacity duration-300 ease-in-out ${isOpen ? 'block opacity-0' : 'hidden opacity-100'}`} onClick={handleCancel}>
