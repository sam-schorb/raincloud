...
function EditMetadataModal({ isOpen, closeModal, patchId, fetchPatchInfo }) {
        ...
        if (isOpen && patchId) {
            fetch(`/getFullPatchInfo/${patchId}`)
                ...
                .then(data => {
                    ...
                    setSelectedImage(null); // This might need additional logic for images
                });
        }
    }, [isOpen, patchId]);
    ...
    const handleImageUpload = (event) => {
        ...
        if (file && file.type === 'image/jpeg') setSelectedImage(file);
        else alert('Please select a valid JPG image.');
    };
